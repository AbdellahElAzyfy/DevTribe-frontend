...
function RightSidebar() {
  ...
  } = useListPosts({ sortBy: "top", limit: 5 });
  const {
  ...
  const trending = (Array.isArray(trendingPosts) ? trendingPosts : []).slice(
    0,
    5,
  );
      ...
      <div className="relative z-10 flex h-full min-h-0 flex-col gap-4 overflow-y-auto p-6">
        ...
        <div className="dash-frame rounded-lg border-2 border-transparent [--dash-color:rgba(100,116,139,0.4)] bg-slate-900/30 p-3">
          ...
          <div className="flex flex-col gap-2 pt-3">
            ...
            {!isTrendingLoading && !isTrendingError && !trending.length ? (
              <p className="px-2 py-2 text-xs text-slate-400">
              ...
              ? trending.map((post) => (
                  ...
                  >
                    ...
                    <div className="mt-1 text-[10px] text-slate-400">
                      {post.community?.slug ? `r/${post.community.slug} · ` : ""}
                      {formatCompactCount(post.voteCount || 0)} votes
                    </div>
                  </Link>
                ))
              : null}
